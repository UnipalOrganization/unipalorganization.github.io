...
let globalVariantIndex: number | null = null;

function getNextVariant(): PathData {
  if (globalVariantIndex === null) {
    globalVariantIndex = Math.floor(Math.random() * pathDataVariants.length);
  } else {
    globalVariantIndex = (globalVariantIndex + 1) % pathDataVariants.length;
  }
  return pathDataVariants[globalVariantIndex];
}

export interface SquigglyHighlightProps {
  children: ReactNode;
  variant?: "underline" | "strikethrough";
  /**
   * Any CSS color. Use a Tailwind CSS variable for theme colors,
   * e.g. color="var(--primary)" or color="var(--soft-foreground)".
   * Defaults to currentColor.
   */
  color?: string;
  /**
   * Stroke width of the squiggle. Defaults to 10 (in path viewBox units).
   */
  strokeWidth?: number;
  /**
   * Delay the draw animation (seconds).
   */
  delay?: number;
  /**
   * Duration of the draw animation (seconds). Defaults to 0.6.
   */
  duration?: number;
  className?: string;
  /**
   * Override the randomised variant with a specific index (0-5).
   */
  variantIndex?: number;
}

export function SquigglyHighlight({
  children,
  variant = "underline",
  color = "currentColor",
  strokeWidth = 10,
  ...
  useEffect(() => {
    if (variantIndex !== undefined) {
      const idx = Math.max(
        0,
        Math.min(pathDataVariants.length - 1, variantIndex)
      );
      setPathData(pathDataVariants[idx]);
    } else {
      setPathData(getNextVariant());
    }
  }, [variantIndex]);
